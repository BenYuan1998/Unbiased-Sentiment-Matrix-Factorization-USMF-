# -*- coding: utf-8 -*-
"""
Created on Mon Dec 14 19:10:37 2020
...
from Renew_Set_MF_IPS import renew_set_MF_IPS
from TrainingAlgorithm_MF_IPS import trainingalgorithm_MF_IPS
import numpy as np
import pandas as pd
import Ranking_Based_on_Relevance
import Get_UnbiasedDCG 
import Get_UnbiasedRecall_MF_IPS


class hyperparametertuning_train_validation_split_MF_IPS(object):
    def __init__(self,df_train,expo_prob,hp_combinations,m):
        """
        Parameters
        ----------
    ...
    def train_validation_split_unbiasedNDCG(self,step=100):
        for hp_combination in self.hp_combinations:
            df_train=renew_set_MF_IPS(self.df_train,self.expo_prob)
            df_train_hptuning=training_set(df_train,self.m)
            training_indices=df_train_hptuning.index.tolist()
            df_validation_hptuning=test_set(df_train,training_indices)
            trainingalgorithm=trainingalgorithm_MF_IPS(hp_combination)
            ...
            for useri in unique_user_indices_validation:
                ...
                for useri_item_interaction in df_validation_hptuning_useri.itertuples():
                    ...
                    rating_predicted=np.dot(trainingalgorithm.user_feature[user_index],trainingalgorithm.item_feature[item_index])
                ...
                useri_list_ranking_predicted=Ranking_Based_on_Relevance.ranking_based_on_relevance(useri_list_rating_predicted)
                useri_list_ranking_actual=Ranking_Based_on_Relevance.ranking_based_on_relevance(useri_list_rating_actual)
                unbiased_DCG=Get_UnbiasedDCG.get_unbiasedDCG(useri_list_ranking_predicted,useri_list_rating_actual,useri_list_expo_prob)
                unbiased_DCG_ideal=Get_UnbiasedDCG.get_unbiasedDCG(useri_list_ranking_actual,useri_list_rating_actual,useri_list_expo_prob)
    ...
    def train_validation_split_unbiasedrecall(self,k,step=100):
        for hp_combination in self.hp_combinations:
            ...
            for useri in unique_user_indices_test:
                ...
                unbiased_recall_useri=Get_UnbiasedRecall_MF_IPS.get_unbiasedrecall_MF_IPS(df_validation_hptuning,useri,user_feature,item_feature,threshold,k)
